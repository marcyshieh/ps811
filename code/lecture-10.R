...
# time frame error in the paper: it claims this is the time frame from black to black to roberts but it seems to be the time frame between black to alito (did not realize they also included alito in their analysis lol)
...
poole_select <- poole %>%
  select(congress, idno, statenm, name, dwnom1)
...
alito_poole$nominee <- rep("ALITO")
alito_poole$presprty <- rep(1)
alito_poole$sameprty <- rep(1)
alito_poole$nomid <- rep(0.100) # https://en.wikipedia.org/wiki/Segal%E2%80%93Cover_score
alito_poole$regime_dummy <- rep(1)
alito_poole$lackqual <- rep(1-0.810) #https://en.wikipedia.org/wiki/Segal%E2%80%93Cover_score
# you will need to calculate the following (later)
alito_poole$csnom <- NA
alito_poole$eucldist <- NA
alito_poole$interaction <-NA
